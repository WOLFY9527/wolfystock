...
"""
    ...
    lines.append("")
    lines.append("> 上方状态来自 PR Review workflow 的 auto-check job，仅覆盖语法正确性（py_compile）和致命 lint 错误（flake8 E9/F63/F7/F82）。仓库主 CI 的 `backend-gate` job 会运行 `./scripts/ci_gate.sh`，但本脚本默认未读取该 job 的实时结果；若 PR 描述或可见检查列表未提供 `backend-gate` / `ci_gate` 结果，应标注为“无法从本段确认”，不得断言该 gate 未纳入 CI。语法/flake8 已通过则无需重复贴对应本地输出。")
    lines.append("")
    return '\n'.join(lines)
...
- Python 后端改动的 PR 描述或可见检查列表未说明 `backend-gate` / `./scripts/ci_gate.sh` 结果
- 描述中非关键性措辞或格式问题
- 注释语言风格、无关锁文件变更等
