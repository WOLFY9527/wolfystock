...
| `workspace-page--chat` | Legacy route residue | Removed standalone chat workspace modifier | Low | Source search proves no active `/chat` route owner or frontend ChatPage remains. | Do not use `/zh/chat` as active route proof; verify redirects through route tests if needed. |
...
- Routes: /zh, /zh/scanner, /zh/watchlist, /zh/backtest, /zh/backtest/results/1,
  ...
  /zh/admin/logs, /zh/admin/notifications, /zh/__preview/report
