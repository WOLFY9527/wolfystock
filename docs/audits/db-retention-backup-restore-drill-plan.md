...
### 3.1 Backend Owner/Domain Lifecycle Matrix

This matrix is a compact ownership/lifecycle overlay for backend persistence and
artifact domains. It documents current coverage and safe future shapes only. It
does not approve retention jobs, DB cleanup, migrations, restore automation, or
production readiness.

| Domain | Owner / table family (known or inferred) | Current coverage | Missing lifecycle / retention / capacity contract | Safe future shape | Hard stops / protected semantics |
| --- | --- | --- | --- | --- | --- |
| Admin logs | Admin log storage and admin audit/log tables | Covered. Storage summary, capacity estimate, dry-run cleanup default, write-capability gate, and focused tests already exist. | Production acceptance of thresholds, operator cadence, and backup/restore drill linkage. | Keep preview-first retention with explicit policy version, capacity summary, protected-row reasons, and gated destructive mode only after separate approval. | Do not weaken write-capability gate, shorten security/audit preservation by accident, or imply cleanup is enabled in production. |
| Durable task state / progress | Durable task rows and progress-event table family used by task polling/replay | Partial. Batch A index/readiness exists, but lifecycle handling is still future work. | Owner-aware terminal retention, progress compaction boundary, active-row protection, and capacity reporting. | Separate task-state retention from progress-event compaction; keep terminal summaries before any bulky progress cleanup. | Do not delete active/running/leased rows, alter polling semantics, or turn retention into stale-task repair. |
| LLM usage / cost ledger / provider usage | Cost/usage observability rows, pricing snapshot references, provider diagnostics/usage ledger families | Partial. Read-only observability posture exists; no accepted retention/capacity contract yet. | Raw-vs-aggregate retention split, owner/route/model aggregation contract, bounded diagnostics capacity rules, and pricing-reference preservation. | Keep observability-only labels, aggregate before any raw-row cleanup, and retain referenced pricing metadata. | Do not turn retention into quota enforcement, secret/payload logging, provider-order changes, or live-call routing changes. |
| Scanner artifacts | Scanner run, candidate, and related diagnostic artifact families | Partial. Growth surface is recognized, but lifecycle policy is not accepted. | User-visible run retention, diagnostic compaction boundary, owner isolation, and capacity dashboard contract. | Preserve user-visible runs longer than bulky diagnostics; allow preview-only candidate cleanup after summary fields are retained. | Do not change scanner scoring, selection, rankings, or broaden provider fanout under a retention task. |
...
Read first: docs/archive/audits/db-index-migration-plan-auth-task-log.md and docs/audits/db-retention-backup-restore-drill-plan.md.
