...
| `docs/audits/provider-fallback-budget-reporting-design.md` | completed, `76288b26` | Future read-only reporting for provider fallback depth, quota risk, timeout, insufficient payload, cache, and inflight efficiency. | Reporting only; no provider order, fallback, timeout, retry, circuit, MarketCache, scanner, backtest, portfolio, AI, notification, or DuckDB changes. | Cost audit, metrics design, admin summary design. | Provider fallback counters Phase 1B and provider fallback reporting section. |
| `docs/audits/wolfystock-post-batch-integration-qa.md` | completed, `f4c5827d` | QA/reporting context confirming recent work and cost-observability sequencing. | QA/report only; no behavior changes intended. | Recent main-branch dashboard, staging, localization, and docs commits. | Confirms instrumentation-first, read-only summary second, cache prototypes after measurement. |
...
- Metrics design -> provider fallback reporting, MarketCache reporting, scanner AI cache design, guest preview reuse, report output cache design, and implementation roadmap.
...
- Provider fallback budget reporting -> provider fallback counters first, then read-only report integration.
...
| Provider fallback counters Phase 1B | partial | Metrics design; provider fallback reporting design; stable helper from Phase 1A if reused | Phase 1A helper/interface decisions and concurrent provider instrumentation edits | MarketCache/scanner counter planning once helper shape is stable | medium/high | Observe provider attempts, fallback depth, quota-risk buckets, cache hits/misses, and inflight joins without changing provider behavior. |
