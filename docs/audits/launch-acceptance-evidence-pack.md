...
| `config_snapshot_evidence` | Accepted sanitized config snapshot evidence from `scripts/config_snapshot_evidence_check.py` as collected through `docs/audits/operator-evidence-real-runbook.md`; auth/provider/quota/database summaries recorded; production mode, auth, CORS/CSRF, trusted proxy, MFA scope, RBAC fallback, quota, provider credential presence, backup/PITR, staging ingress, public SearXNG/public instance, and crypto realtime decision posture are represented only as sanitized labels or presence states; no raw config/env values, no runtime mutation, `releaseApproved=false`, `publicLaunchReady=false`, and manual review required. |
