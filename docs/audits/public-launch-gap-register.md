...
Recent evidence surfaces are reconciled as review-only attachments:

| Evidence surface | Launch/blocker category | Operator artifact |
| --- | --- | --- |
| RBAC R5 fallback observe evidence | `security_operator_acceptance` and `rbac_fallback_disable_switch` review context | `security_operator_acceptance.json` |
| MFA recovery-code acceptance evidence | `security_operator_acceptance` and `mfa_pilot_acceptance` review context | `security_operator_acceptance.json` |
| Restore/PITR real isolated drill operator evidence | `restore_pitr_operator_evidence` and `real_isolated_postgresql_restore_pitr` | `restore_pitr_operator_evidence.json` |
| High-growth retention dry-run evidence | `admin_log_retention_capacity_rehearsal` | sanitized retention dry-run report references |
| API abuse/request-safety evidence | `api_abuse_request_safety` | `api_abuse_safety_evidence.json` |
| Notification delivery rehearsal evidence | `notifications_delivery_rehearsal` | `notification_delivery_rehearsal_evidence.json` |

These artifacts must be filled from real target-environment observations and
reviewed manually. Generated templates, local synthetic fixtures,
dry-run/preflight output, and checker success are not accepted production
evidence; accepted review evidence still keeps `releaseApproved=false`,
`publicLaunchReady=false`, and the public launch verdict **NO-GO** until manual
release review accepts every hard blocker.
...
| Security | Security/Auth | Recovery codes | P0 | Offline MFA recovery-code acceptance evidence can now be reviewed through `security_operator_acceptance.json` / `breakGlassRecovery`, but real target-environment recovery-code operations are not launch-accepted. | Public admin access needs display-once, rotation, single-use consumption, fallback login contract, recovery policy evidence, and manual release review. | Add/accept recovery-code login/session contract and sanitized operator evidence for generation, display-once, hash storage, consume-once, replay denial, rotation/revocation, default-off break-glass, rollback, and audit redaction. | Admin lockout or weak fallback path during MFA rollout. | Auth API tests, replay/consume-once tests, redaction scan, rollback runbook, sanitized operator artifact with `releaseApproved=false` and `runtimeDefaultUnchanged=true`. |
...
| Security/RBAC | Security/RBAC | Coarse fallback removal | P0 | Coarse admin compatibility fallback remains intentional; `scripts/security_operator_acceptance_check.py` can validate sanitized `rbacFallbackObserve` evidence and fallback-off pilot evidence with complete route inventory, backend explicit-capability classification, frontend fail-closed capability gates, explicit capability payload pass proof, legacy/missing-payload fail-closed proof, rollback, sanitized audit evidence, and unchanged runtime defaults. | The fallback remains enabled by default and compatibility code remains present until operators apply the guarded disable switch. Observe evidence is not fallback removal. | Apply `WOLFYSTOCK_ADMIN_RBAC_COARSE_FALLBACK_ENABLED=false` only through the guarded operator path after route inventory stays complete; do not delete compatibility code. | Least-privilege admin model is not enforceable for broad public admin exposure until the switch is accepted and applied. | Route/frontend inventory, fallback-only grant report, sanitized deny/allow audit, pilot rollback proof. |
...
| DB/deployment | Platform/DBA | Retention tiers | P1 | Admin log cleanup now has explicit `admin_logs_standard`, `admin_logs_minimum_protected`, and `admin_logs_storage_pressure` policy evidence with preview-first cleanup, minimum-retention safeguard, sanitized cleanup audit event, storage-size-unavailable fallback, redaction, and warning+ default-view coverage; `docs/audits/high-growth-retention-tier-policy.md` now defines policy-only tiers for major high-growth artifact classes, and `admin_log_retention_capacity_rehearsal` tracks retention dry-run evidence in the launch matrix. | Accepted dry-run reports and cleanup approval remain missing for task progress, terminal task state, LLM usage, scanner/backtest artifacts, provider counters, guest/cache metadata, future Options cache rows, and other non-admin-log domains. | Pilot preview-first owner/domain-aware dry-run reports for the remaining non-admin-log domains while keeping `deleteAllowed=false` until separately approved. | Unbounded growth, privacy retention drift, expensive operations under public load. | Admin-log focused pytest evidence plus dry-run cleanup reports, broader domain retention matrix, owner-aware deletion checks, rollback notes. |
| DB/deployment | Platform/DBA | Backup/restore drill | P0 | Local dry-run preflight, restore/PITR operator templates, and offline validators now provide a reviewable evidence path with explicit `NO-GO` defaults and no restore execution by repo helpers; real isolated PostgreSQL restore execution remains missing. | Encrypted backup, PITR execution, restore smoke, accepted sanitized operator evidence, and rollback proof are not exercised. | Create and run an isolated PostgreSQL backup/restore drill with synthetic data after the preflight passes with fresh synthetic or sanitized metadata, then validate only sanitized real-drill and operator evidence artifacts. | Data loss or untested recovery during launch incident. | Preflight output, restore smoke, PITR target evidence, sanitized operator validator output, rollback decision point, owner-isolation smoke after restore. |
| DB/deployment | Platform/DBA | Retention tiers | P1 | Admin log cleanup now has explicit `admin_logs_standard`, `admin_logs_minimum_protected`, and `admin_logs_storage_pressure` policy evidence with preview-first cleanup, minimum-retention safeguard, sanitized cleanup audit event, storage-size-unavailable fallback, redaction, and warning+ default-view coverage; policy-only high-growth retention tiers exist, but accepted dry-run reports remain missing for non-admin-log domains. | Task progress, terminal task state, LLM usage, scanner/backtest artifacts, provider counters, guest/cache metadata, and future Options cache retention still need accepted dry-run report evidence before cleanup approval. | Design and pilot preview-first owner/domain-aware retention dry-run reports for the remaining non-admin-log domains. | Unbounded growth, privacy retention drift, expensive operations under public load. | Admin-log focused pytest evidence plus dry-run cleanup reports, broader domain retention matrix, owner-aware deletion checks, rollback notes. |
...
| DB/deployment | Platform/Release | Staging smoke | P0 | A safe dry-run/live opt-in ingress preflight and `scripts/staging_ingress_operator_evidence_check.py` now separate local dry-run/preflight/template output from accepted sanitized target-environment HTTPS ingress artifacts. Accepted staging ingress evidence must use `evidenceMode=target-environment-https-ingress`; local dry-run, preflight, or template evidence remains review-only and cannot satisfy accepted target-environment HTTPS ingress evidence. Accepted artifacts must keep `releaseApproved=false`, `publicLaunchReady=false`, rollback/manual review state, synthetic user/data posture, and sensitive/debug payload redaction. Real HTTPS staging evidence is still not accepted. | HTTPS reverse proxy, synthetic users/data, no direct public backend `:8000`, owner isolation, rollback checks, public 80/443-only exposure, HTTP-to-HTTPS redirect posture, health/readiness/live summaries, protected admin 401/403 fail-closed summary, and sensitive/debug payload redaction are not proven together in staging. | Run `WOLFYSTOCK_STAGING_INGRESS_SMOKE=1 python3 scripts/staging_ingress_smoke.py --base-url "$STAGING_INGRESS_BASE_URL"` against synthetic staging, attach JSON evidence, and validate the sanitized target-environment operator artifact offline. Reject raw URLs with secrets, credential-bearing URLs, private hostnames or IP-sensitive labels, raw/debug payload markers, secret/header markers, runtime mutation claims, launch approval wording, `releaseApproved=true`, or `publicLaunchReady=true`. | Deployment can pass local checks but fail under ingress, proxy, auth, or owner routing; unsafe evidence can also be mistaken for launch approval. | Staging smoke report, ingress/port proof, HTTP-to-HTTPS redirect proof, health/readiness/live checks, protected admin 401/403, owner isolation, rollback note, manual review state, redacted evidence proof, no-secret scan. |
| DB/deployment | Release owner | Production config contract | P0 | A safe production config/secret contract preflight now exists for required launch flag names, explicit production mode, `ADMIN_AUTH_ENABLED=true`, CORS/CSRF posture, trusted proxy posture, MFA rollout mode, RBAC fallback evidence, provider credential presence states, quota mode, backup/PITR opt-in, staging ingress opt-in, public SearXNG/public instance posture, and crypto realtime decision posture. `scripts/config_snapshot_evidence_check.py` also validates sanitized config snapshot summaries offline with sanitized labels and presence states only, including `releaseApproved=false` and `publicLaunchReady=false`. Neither helper reads raw `.env` values or implies any runtime config/default change. | A real production config contract and config snapshot artifact have not been accepted, and raw `.env`, DSN, provider key, token, cookie, password, webhook, private hostname, stack trace, or other secret values must not be attached to release evidence. Auth-disabled config remains local/dev-only, public SearXNG/public instance posture remains unsafe for production readiness, and public launch remains **NO-GO**. | Run `python3 scripts/production_config_readiness.py --contract <sanitized-production-config-contract.json>` and `python3 scripts/config_snapshot_evidence_check.py <sanitized-config-snapshot-evidence.json>` with names/presence states only, attach JSON evidence, and keep missing required config, missing/false `ADMIN_AUTH_ENABLED`, enabled public SearXNG discovery, unresolved accepted posture, `releaseApproved=true`, or `publicLaunchReady=true` as NO-GO. | Launch reviewers can otherwise confuse defaults, missing flags, auth-disabled local mode, public-instance discovery, realtime provider posture, and credential readiness with accepted production posture. | Stable JSON output, sanitized labels/presence states only, no raw secrets, explicit NO-GO on missing required config, no external calls, runtime defaults unchanged. |
| WS2/multi-instance | Runtime/Platform | Staging/CI smoke acceptance | P0 | Smoke design exists and repo-local offline smoke tooling is available through `scripts/ws2_multi_instance_smoke.py` for dry-run and disposable-SQLite synthetic preflight. `scripts/ws2_target_environment_evidence_check.py` and `docs/audits/ws2-target-environment-evidence-template.json` now provide an offline review path for operator-supplied sanitized target-environment evidence, with `PROFILE_WS2_ACCEPTANCE_EVIDENCE_SCOPED` output that separates API A submit, worker lease/claim/progress, API B durable status readback, polling replay, owner-hidden status/polling, lease expiry recovery, retry/failure safety, and process-local SSE limitation handling. The target-env artifact is also part of the operator evidence template pack, bundle checker, and schema reference, and requires explicit deployment topology label, rollback/degraded note, manual review status, single-instance exception posture when used, `sseBroadcastScope=process-local`, durable polling as the baseline, `externalSseReplayImplemented=false`, `releaseApproved=false`, and `publicLaunchReady=false`. These tools do not run staging/API calls, prove PostgreSQL API A/B topology by themselves, implement external SSE replay, add broker/queue cutover, change task/SSE/worker runtime behavior, or mark target-environment evidence launch-ready. | No accepted staging/API A/B WS2 evidence proves API A submit, worker lease/claim/progress, API B durable read, durable polling replay, owner-hidden polling, lease expiry recovery, retry/failure safety, and owner isolation. Placeholder, local, fixture-only, CI synthetic, cross-instance SSE reliability claims without evidence, runtime cutover claims, or public-launch approval wording remain insufficient. | Run/extend WS2 smoke toward CI and staging acceptance using fake provider/LLM, synthetic users, disposable storage, and sanitized operator artifacts only; validate the real target-environment artifact offline after operators fill every scoped acceptance dimension. Reject raw URLs, tokens, cookies, user/session IDs, task payload bodies, stack traces, provider payloads, private hostnames, queue/broker credentials, cross-instance SSE reliability claims without evidence, runtime cutover claims, `releaseApproved=true`, `publicLaunchReady=true`, and public launch approval wording. | Multi-instance deployment can lose task visibility or leak owner state if offline preflight, validator success, or process-local SSE behavior is mistaken for target-environment proof; durable polling remains the multi-instance baseline until separately implemented external replay/cutover evidence exists. | Offline synthetic preflight output, disposable PostgreSQL smoke if implemented, CI synthetic smoke, accepted staging API A/B artifact, PASS/FAIL table, sanitized target-environment checker output, scoped acceptance-dimension matrix, sanitized operator review, redacted evidence proof. |
...
| Cost/quota | Cost/Quota | Live enforcement pilot | P0 | Quota dry-run/reservation helpers, pilot-readiness preflight, and `scripts/quota_operator_evidence_check.py` now provide offline validation/templates for sanitized quota/budget operator evidence. A default-off, owner-allowlisted sync single-stock analysis pilot can block only that route on quota rejection, consume estimated route units after success, and release on analysis failure. Cost-ledger reservation evidence now separates the existing `quota_reservation_id` ledger seam from route-estimated pilot consumption. | Operator/staging evidence, admin visibility, owner/guest accounting acceptance, invoice reconciliation, a single terminal transition owner, exact-once actual-cost consume, and broad/global enforcement remain incomplete. | Attach accepted sanitized operator evidence for the narrow route pilot, then separately decide whether route-pilot terminal handling or cost-ledger reconciliation owns final reservation consume before any broader spend-cap expansion. | Public usage can still run without accepted broad hard spend-cap readiness, and estimated route units must not be treated as billing-authoritative actual provider cost. | Reservation lifecycle tests, cost-ledger reservation evidence tests, would-block behavior, rollback flag, user/admin copy showing enforcement status, staging/operator evidence, no public-launch approval wording. |
...
| Provider reliability | Provider/Ops | Circuit enforcement pilot | P0 | Circuit dry-run observer exists, a default-off admin built-in provider validation probe pilot can block only `data_source_validation/admin_provider_probe` when explicitly enabled with rollback available, and admin SLA readiness can expose opt-in sanitized `adminProbePilotEvidence` for pilot state, rollback, selected boundary, and last decision category. | No public/user provider runtime integration is approved for provider ordering, fallback, retry, timeout, in-flight, sufficiency, or MarketCache behavior. The admin probe pilot and its evidence surface are not launch acceptance. | Attach accepted operator/staging evidence for the admin probe pilot, then separately design any broader public-route provider circuit policy without changing global fallback semantics by accident. | Provider outages or 429s can still cascade through public traffic outside the narrow admin probe boundary. | Synthetic timeout/429/403/5xx buckets, default-off proof, rollback switch, no raw URL/payload/credential exposure, readiness degraded state, explicit no-public-launch-approval wording. |
...
| Portfolio/backtest | Broker/Security | Broker credential redaction audit | P0 | Synthetic broker/token/session redaction evidence now covers user broker-connection metadata, targeted portfolio import preview/commit response artifacts, offline broker/order/trade/import-export artifact sanitization, admin portfolio export-like surfaces, and release secret-scan coverage; `docs/audits/broker-order-trade-redaction-release-evidence-checklist.md` tracks the tested boundaries and remaining evidence. | Broker logs, full browser-visible panels, owner-visible field classification, full sync/import failure surfaces, and release-candidate exports still need full redaction proof. | Complete broker redaction audit across API/log/admin/browser/export surfaces; fix only confirmed leaks in separate scoped tasks. | Raw broker credentials, tokens, sessions, account IDs, order IDs, request IDs, URLs, payloads, or account metadata could leak. | API/log/admin/browser/export redaction scan, import preview/commit redaction evidence, synthetic secret fixtures only, no production credential inspection. |
