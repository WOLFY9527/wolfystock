...
| DB/deployment | Platform/Release | Staging smoke | P0 | A safe dry-run/live opt-in ingress preflight and `scripts/staging_ingress_operator_evidence_check.py` now separate local dry-run output from accepted sanitized target-environment HTTPS ingress artifacts. Accepted artifacts must keep `releaseApproved=false` and `publicLaunchReady=false`. Real HTTPS staging evidence is still not accepted. | HTTPS reverse proxy, synthetic users/data, no direct public backend `:8000`, owner isolation, rollback checks, public 80/443-only exposure, and HTTP-to-HTTPS redirect posture are not proven together in staging. | Run `WOLFYSTOCK_STAGING_INGRESS_SMOKE=1 python3 scripts/staging_ingress_smoke.py --base-url "$STAGING_INGRESS_BASE_URL"` against synthetic staging, attach JSON evidence, and validate the sanitized target-environment operator artifact offline. | Deployment can pass local checks but fail under ingress, proxy, auth, or owner routing. | Staging smoke report, ingress/port proof, HTTP-to-HTTPS redirect proof, health/readiness/live checks, protected admin 401/403, owner isolation, redacted evidence proof, no-secret scan. |
