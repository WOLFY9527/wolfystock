...
| WS2/multi-instance | Runtime/Platform | Staging/CI smoke acceptance | P0 | Smoke design exists and repo-local offline smoke tooling is available through `scripts/ws2_multi_instance_smoke.py` for dry-run and disposable-SQLite synthetic preflight. `scripts/ws2_target_environment_evidence_check.py` and `docs/audits/ws2-target-environment-evidence-template.json` now provide an offline review path for operator-supplied sanitized target-environment evidence, with `PROFILE_WS2_ACCEPTANCE_EVIDENCE_SCOPED` output that separates API A submit, synthetic worker/lease flow, API B durable status readback, polling replay, owner-hidden status/polling, retry/failure safety, and SSE limitation handling. These tools do not run staging calls, prove PostgreSQL API A/B topology by themselves, or mark target-environment evidence accepted. | No accepted staging/API A/B WS2 evidence proves API A submit, worker lease, API B durable read, durable polling replay, retry/failure safety, and owner isolation. Placeholder, local, fixture-only, or CI synthetic evidence remains insufficient. | Run/extend WS2 smoke toward CI and staging acceptance using fake provider/LLM, synthetic users, disposable storage, and sanitized operator artifacts only; validate the real target-environment artifact offline after operators fill every scoped acceptance dimension. | Multi-instance deployment can lose task visibility or leak owner state if offline preflight or validator success is mistaken for target-environment proof. | Offline synthetic preflight output, disposable PostgreSQL smoke if implemented, CI synthetic smoke, accepted staging API A/B artifact, PASS/FAIL table, sanitized target-environment checker output, scoped acceptance-dimension matrix, sanitized operator review. |
