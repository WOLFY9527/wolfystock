...
| Eastmoney unverified HTTPS warning | Non-blocking | Market data provider / Eastmoney adapter | Provider warning can appear during data-source calls; not launch-blocking by itself, but it should not be hidden if it indicates insecure transport or certificate validation drift. | Reproduce in an isolated provider smoke without real credentials, identify the exact library call stack, prefer verified HTTPS/certificate configuration, and only suppress the warning if the provider library cannot be changed and the risk is documented. |
