...
| Liquidity Monitor | `/market/liquidity-monitor`; T-1757 WS-029 | `GET /api/v1/market/liquidity-monitor` | Cache-only reads from crypto, volatility, rates, macro, FX/commodities, funds-flow, breadth, CN/HK, futures panels | No external provider calls by default; regime unavailable when reliable indicator count is under threshold or confidence is too low | Coverage/provider activation issue, not route wiring. T-1762 reconciled coverage as 12 indicator families and 39 named required input slots; the legacy "1 of 49" denominator maps to score-weight budget, not input coverage | High | Provider auth, cache freshness, paid/official data, possible source-review boundary | Use the additive coverage contract to compare required, fulfilled, missing, score-eligible, and observation-only inputs per family; then activate real sources by family | `python -m pytest tests/api/test_market_intelligence_payload_smoke.py -k liquidity -q` | Coverage report parallel; provider activation serial/manual |
...
### WS-029: Liquidity Monitor Coverage Denominator Reconciled
...
T-1762 reconciliation: the public contract now exposes required-input coverage as 12 indicator families and 39 named required input slots. The numeric value 49 remains a separate score-weight budget, not an indicator count or required-input denominator.

Confidence: High.

Next diagnostic: use the additive coverage contract to review each indicator family, required input, fulfilled input, missing input, score eligibility, and observation-only input state before any real-source activation.
...
- T-1757's "1 of 49" Liquidity denominator should be treated as stale wording after T-1762; use 39 required input slots for coverage and 49 only for score-weight budget.
