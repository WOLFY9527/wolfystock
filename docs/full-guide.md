...
| `ADMIN_AUTH_ENABLED` | Web 登录：生产、Docker、云服务器部署必须保持 `true`；本地开发如需无密码调试可临时设为 `false`。首次访问在网页设置初始密码，可在「系统设置 > 修改密码」修改；忘记密码执行 `python -m src.auth reset_password` | `true` |
| `CORS_ALLOW_ALL` | 仅限本地开发排查跨域时临时设为 `true`；生产请保持 `false` 并配置明确的 CORS 允许来源 | `false` |
...
- support export index 里对应的 `execution_trace_csv` 现在也有了真实的只读 CSV API export：单条规则回测可以直接拿到表格友好的 execution-trace CSV，而不必再依赖 service 侧写文件导出。它和 JSON trace export 一样复用当前 stored-first execution trace 读回逻辑，并在 trace rows 缺失时明确返回 unavailable。
...
2. 点击 `打开比较工作台`，确认页面导航到 `/backtest/compare?runIds=...`，并能稳定展示 baseline、`comparison_summary`、`parameter_comparison`、`period_comparison`、`market_code_comparison`、`robustness_summary`、`comparison_profile` 与 `comparison_highlights`。
...
- `parsed_strategy.strategy_spec.risk_controls.{stop_loss_pct,take_profit_pct,trailing_stop_pct}`：若自然语言里写出固定百分比止损 / 止盈 / 移动止损（如 `止损5%`、`止盈10%`、`移动止损8%`），会显式保留在结构化策略规格里，并按“收盘触发、下一根 bar 开盘离场”执行；当前仍只支持单仓位、单标的、百分比阈值版本，不扩展到参数优化或多资产语义
