...
- `POST /api/v1/backtest/rule/universe-jobs` creates a stored local-only universe job scaffold. This phase performs preflight only: it deduplicates and sorts normalized symbols, persists `sequence_index`, records compact per-symbol readiness rows, and defaults to a 500-symbol cap. Symbols without local daily bars are marked `skipped / blocked_missing_local_data`. The endpoint does not execute single-symbol rule backtests, call provider hydration, write heavy run details, enable worker concurrency, or make DuckDB a runtime source of truth.
- `GET /api/v1/backtest/rule/universe-jobs/{job_id}/status` returns lightweight universe job metadata and progress counters; `GET /api/v1/backtest/rule/universe-jobs/{job_id}/results` returns compact symbol rows ordered by `sequence_index` with a page-size cap of 100, so large universes are not returned in one payload.
