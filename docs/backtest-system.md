...
- `POST /api/v1/backtest/rule/universe-jobs` 创建 stored local-only universe job scaffold。当前阶段仅做本地日线数据预检：按规范化代码去重并排序、持久化 `sequence_index`、记录 compact per-symbol readiness row，并默认限制最多 500 个标的。缺少本地数据的标的会标记为 `skipped / blocked_missing_local_data`。该接口不会执行单标的规则回测、不会调用 provider 拉取、不会写入 heavy run detail、不会启用 worker concurrency，也不会把 DuckDB 作为运行时真源。
- `GET /api/v1/backtest/rule/universe-jobs/{job_id}/status` 返回 universe job 的轻量 metadata / progress counters；`GET /api/v1/backtest/rule/universe-jobs/{job_id}/results` 返回按 `sequence_index` 排序的分页 compact symbol rows，单页上限为 100，避免一次响应塞入大宇宙明细。
