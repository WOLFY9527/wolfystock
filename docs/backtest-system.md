...
- `POST /api/v1/backtest/rule/run` 是 deterministic 的单标的规则策略回测。
  ...
  回测。
- 规则回测执行模型现在有显式 request-side registry gate。省略
  `execution_model` 或显式传入 `{"version":"v1"}` 都继续使用当前/default
  deterministic rule backtest execution model v1；显式 `v2` 或未知模型会在
  API/service 入口 fail closed，早于引擎执行、异步 run 写入、provider 调用与
  export 生成。该 gate 只表达版本支持，不改变现有成交、费用、滑点、benchmark、
  metrics 或 stored result 语义，也不会把 unsupported v2 静默降级成 v1。
...
- `GET /api/v1/backtest/rule/runs/{run_id}/export-index` 提供单条规则回测当前可发现的导出项索引，当前稳定集合是：`support_bundle_manifest_json`、`support_bundle_reproducibility_manifest_json`、`execution_trace_json`、`execution_trace_csv`、`robustness_evidence_json`、`regime_attribution_readiness_json`、`execution_model_metadata_json`、`oos_parameter_readiness_json`。两个 manifest 都会给出可直接读取的 API path；execution-trace JSON/CSV、stored robustness evidence JSON、diagnostic regime-attribution readiness projection JSON、read-only execution-model metadata JSON，以及 OOS/parameter-readiness 组合投影 JSON 现在也都暴露真实的 API path，而不再只是 service-file-only 提示。trace 可用性按当前 detail 读回后的 `execution_trace.rows` 是否非空判断，因此当历史运行缺少可导出的 trace rows 时，index 会稳定返回 `available=false` 与 `execution_trace_rows_missing`；robustness evidence 则按已存 `robustness_analysis` 是否存在返回 `stored_robustness_analysis_present / stored_robustness_analysis_missing`；regime attribution readiness 则继续作为 diagnostic/readiness projection 读回，不代表已具备 validated institutional PnL attribution；execution-model metadata 则固定暴露当前/default rule backtest execution model v1 的只读语义与 guardrails，不会重跑引擎，也不代表已实现 institutional execution realism；OOS/parameter-readiness export 则保持 stored-first，只在已有 stored robustness/walk-forward evidence 或内部 service/helper 调用方提供的 compare/parameter evidence 可投影时暴露对应 section，public GET 本身不接受 compare payload input，缺失一侧会明确返回 partial/unavailable 诊断而不是伪造证据。
...
- `GET /api/v1/backtest/rule/runs/{run_id}/execution-model-metadata.json` 返回当前/default rule backtest execution model v1 的只读 metadata export。这个 payload 明确声明现阶段仍是既有 deterministic rule-backtest engine 行为，fee/slippage 仍只是 assumptions if present，同时继续显式标记无 market impact、无 spread simulation、无 partial fills、无 halt/limit-up/limit-down 模型、无 tax/stamp-duty 模型、无 volume participation cap、无 PIT universe / adjusted-data guarantee、无 provider/live call requirement，并保持 `diagnostic_only/readiness_only/decision_grade=false`。同一 export 还包含 registry 摘要：`v1` 是唯一 supported/current/default executable model，`v2` 只是 unavailable/fail-closed 的 future posture。
- `GET /api/v1/backtest/rule/runs/{run_id}/oos-parameter-readiness.json` 返回单条规则回测的 OOS 与 parameter-readiness 组合导出。它是 diagnostic/readiness only 的 stored-first projection：OOS section 只读取已存 `robustness_analysis.walk_forward` 派生出的 `walk_forward_oos_evidence`；parameter section 只读取内部 service/helper 可选传入的 compare/parameter stability evidence 投影。该 public GET 只有 `run_id` 读取语义，不接受 compare payload、parameter evidence 或 request body input；接口不会重跑策略、优化器或参数扫描，不会触发 provider/live 调用，也不会改动已存 run result。若某一侧证据不存在，对应 section 会显式返回 `unavailable`，整体 state 也只会落在 `partial/unavailable`，不会伪造 readiness。
