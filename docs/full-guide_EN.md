...
| `CN_HK_CONNECT_FLOW_API_KEY` | Authorized CN/HK Connect Flow credential placeholder; the current diagnostic path does not read or return this value | - | Optional |
| `CN_MONEY_MARKET_RATES_CACHE_PATH` | Local JSON cache path for official-public CN money-market diagnostics; no key required, Provider Ops only, and not Liquidity Monitor scoring input | - | Optional |
...
- The compare workbench now also includes a compact sticky section navigator near the top of the results area, with quick links to `Comparison Summary / comparison_highlights / compact metric matrix / robustness + profile / market / period context / parameter + metrics / Participating Runs`, so longer compare pages are faster to navigate without changing the compare route or backend contract.
- The compare workbench now also includes one lightweight visual chart strip. It stays intentionally small: no new backend data, no dashboard expansion, and no heavy charting workflow, just a compact visual layer over the existing trusted compare metrics so users can scan the baseline-versus-candidate picture more quickly.
- The compare workbench now also adds lightweight collapse/expand controls for lower-priority sections. `comparison_highlights`, `market / period context`, `parameter + metrics`, and `Participating Runs` still load expanded by default, but users can temporarily fold them to reduce scroll depth without changing routes, backend behavior, or saved preferences.
- The compare workbench summary area now also exposes a small share/export action group. Users can copy the current compare URL, the active `runIds` selection, or a short compare summary string built from the already rendered baseline/profile/context data, without any backend export endpoint.
- Rule-backtest results now also expose an additive `robustness_analysis` block on the detail/history read surface. It contains a minimal `walk_forward`, `monte_carlo`, and `stress_tests` summary: walk-forward treats rolling train/test windows as execution context only and does not re-optimize parameters, Monte Carlo applies a deterministic-seed perturbation over observed returns, and stress tests run a small fixed shock set. All three stay compact and additive, returning only summary metrics/aggregates without replacing baseline metrics, benchmark data, or execution-trace contracts.
- The Web UI now exposes these additive backtest fields as read-only panels: the `/backtest` confirmation step shows a compact stop-loss / take-profit / trailing-stop summary when `strategy_spec.risk_controls` exists, and `/backtest/results/:runId` shows a compact robustness panel in the `Parameters & Assumptions` tab when `robustness_analysis` is present. This remains additive UI exposure only and does not change execution semantics or the result-page dashboard structure.
- Failed and cancelled rule-backtest runs now also expose a more stable stored-first diagnostics summary. In addition to `no_result_reason`, `no_result_message`, and `status_history`, the status/detail/history read paths now return a structured `run_diagnostics` block that surfaces the terminal status, reason code, last transition timestamp, and the last non-terminal stage before the run stopped, making reopen/debug flows less dependent on transient logs.
- Rule-backtest lifecycle timing now also has a stored-first `run_timing` summary. The status/detail/history read paths consistently expose `created_at`, `started_at`, `finished_at`, `failed_at`, `cancelled_at`, `last_updated_at`, plus queue and execution durations, so reopen/debug flows no longer have to reconstruct timing from scattered `run_at`, `completed_at`, and raw `status_history` entries.
- The export index entry for `execution_trace_json` is now backed by a real read-only API export instead of a service-file-only path. A single rule-backtest run can now return the execution-trace JSON payload directly for AI debugging, automation, backend handoff, and migration workflows, while still truthfully returning unavailable when trace rows do not exist.
- The matching `execution_trace_csv` export is now also backed by a real read-only API endpoint instead of a service-file-only path. A single rule-backtest run can return spreadsheet-friendly execution-trace CSV directly for operator review, automation, handoff, and migration workflows, while still returning unavailable when trace rows do not exist.
...
- `parsed_strategy.strategy_spec.risk_controls.{stop_loss_pct,take_profit_pct,trailing_stop_pct}`: when the natural-language strategy includes a fixed percentage stop-loss / take-profit / trailing stop such as `stop loss 5%`, `take profit 10%`, or `trailing stop 8%`, the normalized strategy spec preserves those values and executes them with the existing semantics of “trigger on close, exit on the next bar open”; the scope still stays minimal: single-position, single-instrument, percentage-threshold only
...
- After confirming logout, the Web app clears local session state and explicitly redirects to `/login`; that redirect path is covered by unit tests. In the local `authEnabled=false` environment, the full live-browser logout redirect cannot be reproduced as an authenticated session, so that browser check is recorded as environment-limited.
- `/reset-password` now calls `/api/v1/auth/reset-password/request` for a non-enumerating reset request and keeps locale plus redirect parameters when linking back to login. With local auth disabled, the protected reset-password end-to-end path cannot be fully verified in the browser and should be marked `authEnabled=false` limited.
- The `/settings/system` AI Provider Library now shows Quick API status and advanced-channel counts on provider cards. Built-in and custom API providers can be managed through the advanced channel editor, including add/edit/delete, connection checks, extra header/parameter maintenance, and saved channel configuration. Safari/live smoke validation covered Admin Mode entry, system-settings navigation, AI provider drawer visibility, and EN/ZH switching; ordinary-user denial checks remain limited by local `authEnabled=false`.
...
- The Web `/portfolio` page now surfaces these three additive blocks as compact read-only attribution cards, so `portfolio_attribution`, `account_attribution`, and `industry_attribution` are visible without changing the existing positions table, concentration chart, risk alerts, or write flows.
