## 2026-06-22
- Consolidated AI onboarding Markdown navigation around a six-file start path, regenerated `docs/AI_PROJECT_MANUAL.md` from durable sources, rewrote `docs/DOCS_INDEX.md` as the active classification map, centralized retired audit/recovery/task reports under `docs/archive/`, and kept `docs/codex/` plus `docs/product-recovery/` focused on durable current references. This is docs/manual-generator maintenance only; no runtime code, product behavior, provider order, credentials, env/config/package files, auth, broker, accounting, portfolio ledger semantics, DB migrations, external network behavior, or frontend/backend product code changed.

## 2026-06-16
- Web frontend information architecture now centers the consumer shell on a Market Structure + Positioning Intelligence workflow: Decision Cockpit becomes the primary market entry, Research Radar and Stock Structure become core research routes, scanner/watchlist/portfolio move under research context, Options/Gamma remains observation-only and non-decision-grade, and Scenario Lab is exposed as a static placeholder entry. Admin/Ops remains separated, no broker/order/trade UI was added, and no backend, provider/runtime/cache/storage/accounting/auth, scanner ranking, portfolio accounting, options ranking/gates, or API contract behavior changed.

## 2026-06-15
- Watchlist now exposes a read-only research overlay at `GET /api/v1/watchlist/research-overlay`, projecting owner-scoped watched symbols into structure state, research-attention priority when evidence is sufficient, verification checks, risk flags, evidence gaps, freshness, aggregate summaries, missing evidence, data quality, and a no-advice disclosure. The endpoint fails closed on unavailable watchlist or OHLCV evidence, does not mutate watchlist membership, does not create alerts or notifications, does not change scanner/radar ranking, and does not touch provider runtime/cache or frontend behavior.
- Market Scenario Lab now exposes `POST /api/v1/market/scenario-lab`, a bounded research-planning API over the existing deterministic scenario engine. The endpoint accepts caller-supplied base regime evidence or normalized driver scores plus named scenarios/overrides, fails closed when base evidence is missing, returns driver/confidence deltas, confirmation/invalidation prompts, evidence limits, and an explicit no-advice disclosure, and does not change provider runtime/cache, auth/RBAC/session, frontend, broker/order/trade, portfolio, scanner, or backtest behavior.
- Scanner now exposes a read-only research overlay at `GET /api/v1/scanner/runs/{run_id}/research-overlay`, projecting finalized scanner candidates into regime fit, theme alignment, evidence quality, verification checks, risk flags, evidence gaps, aggregate data quality, and queue diversity. The endpoint fails closed when candidate evidence is thin and does not run scanners, change candidate generation, change ranking/order, mutate persistence, mutate watchlists or portfolios, call provider/runtime/cache paths, send alerts, or touch frontend behavior.
- Portfolio API now exposes `GET /api/v1/portfolio/structure-review`, an additive read-only overlay that projects cached current holdings through the existing stock structure batch engine. The response includes schema version, aggregate structure health, available theme or sector exposure, structure-state counts, per-holding evidence quality, strongest structures, weakest evidence, common risk flags, missing evidence, data quality, and explicit no-advice disclosure while preserving manual ledger semantics and avoiding provider runtime/cache or frontend changes.
- Research Radar scoring v2 now improves the daily research queue by merging duplicate ticker evidence, preserving deterministic priority/score/evidence/symbol ordering, adding bounded theme/regime/evidence-quality/liquidity/diversity scoring, and exposing additive explanation fields for why-on-radar, verification checks, lower-priority reasons, evidence gaps, duplicate merge count, and queue diversity. This remains research-only/no-advice, does not change scanner production ranking, does not mutate watchlists, and does not touch provider runtime/cache, frontend, auth/RBAC, portfolio/broker/order/backtest, or data-provider behavior.
- Stocks API now exposes `POST /api/v1/stocks/structure-decisions/batch`, an additive observation-only batch contract for multiple stock structure decisions with bounded `maxItems`, stable per-symbol ordering, optional benchmark-based relative-strength ranking, aggregate structure counts, strongest/weakest evidence summaries, common risk flags, and explicit no-advice disclosure. The endpoint remains fail-closed on missing OHLCV evidence and does not change provider runtime/cache internals, watchlist or portfolio mutation, broker/order/trade paths, auth/RBAC/session behavior, scanner/backtest behavior, or personalized-advice boundaries.
- Web frontend now exposes three additive rough-shell routes for Open Design information-architecture handoff without changing backend contracts or the shared global shell: `/market/decision-cockpit` renders a Market Decision Cockpit frame over `GET /api/v1/market/decision-cockpit`, `/stocks/{stock_code}/structure-decision` renders a Stock Structure Decision panel over `GET /api/v1/stocks/{stock_code}/structure-decision`, and `/research/radar` renders an authenticated Research Radar queue over `GET /api/v1/research/radar`. The surfaces stay observation-only / research-only, avoid trade/advice wording, do not add a new design system, do not expand primary navigation, and do not change auth/RBAC, provider/runtime/cache behavior, portfolio/broker/order/backtest behavior, or backend response semantics.
- Research Radar now exposes a read-only backend queue at `GET /api/v1/research/radar`, projecting the existing candidate engine and latest owner-scoped scanner candidates into research priority, verification, evidence-gap, market-fit, data-quality, and no-advice fields. The endpoint fails closed to a degraded empty queue when scanner evidence is unavailable, and it does not run scanners, change scanner ranking, mutate watchlists, call provider/runtime/cache internals, or touch portfolio/broker/order/backtest paths.
- Market Decision Cockpit now adds deterministic decision-support diagnostics to `GET /api/v1/market/decision-cockpit`: driver attribution, confidence diagnostics, watch triggers, current-snapshot-only change notes, cockpit readiness, and text-only scenario hints. The additions are backward-compatible, fail closed on missing evidence, keep options structure observation-only and non-decision-grade, avoid price targets or order language, and do not change provider runtime/cache, scanner ranking, watchlist, portfolio/broker/order/backtest, frontend, or auth/RBAC behavior.

## 2026-06-12
- Market Decision Cockpit backend v1 now exposes a read-only aggregate at `GET /api/v1/market/decision-cockpit`, composing the existing market regime engine output, a preview-only research queue, and observation-only options structure status into one fail-closed decision-support payload. The endpoint remains no-advice, does not guess option-chain evidence when unavailable, and does not change provider runtime, scanner ranking, watchlist mutation, portfolio/broker/order/backtest behavior, or auth/RBAC semantics.
- Stocks API now exposes `GET /api/v1/stocks/{stock_code}/structure-decision`, an additive observation-only structure decision contract backed by existing daily OHLCV history access and the pure Stock Structure Decision Engine. The response includes `schemaVersion`, `ticker`, `structureState`, `confidence`, `componentScores`, `explanation`, `researchNotes`, `dataQuality`, `missingEvidence`, and `noAdviceDisclosure`; missing or insufficient OHLCV fails closed to low-confidence structure with unavailable or insufficient data quality. This does not change provider runtime/cache internals, watchlist or portfolio mutation, broker/order/trade paths, auth/RBAC/session behavior, scanner/backtest behavior, frontend source, or personalized-advice boundaries.
- Staging ingress operator evidence now separates local dry-run/preflight output from accepted sanitized target-environment HTTPS ingress evidence. Accepted artifacts must record HTTPS reverse proxy posture, public 80/443-only exposure, backend `:8000` not directly public, HTTP-to-HTTPS redirect posture, health/readiness/live summaries, protected admin 401/403 fail-closed summary, redacted evidence posture, synthetic user/data posture, owner-isolation or not-applicable summary, rollback note, manual review readiness, and `releaseApproved=false` / `publicLaunchReady=false`. This hardens offline evidence review only; it does not run staging calls, change ingress/deployment/runtime config, change API middleware, change auth/RBAC/session behavior, change frontend behavior, or approve public launch.
- Quota reserve/release operator evidence validation now includes a sanitized accepted-packet posture section and checked-in synthetic valid/invalid artifacts. A passing packet is reviewable only and keeps `publicLaunchReady=false`, `releaseApproved=false`, `launchApproved=false`, no guest/public quota mutation, and invoice/export reconciliation unaccepted/advisory; this does not change live quota enforcement, reservation consume/blocking behavior, cost ledger semantics, provider/runtime behavior, auth/RBAC/session behavior, DB schema, frontend behavior, notifications, broker/order/trade paths, or public-launch **NO-GO** posture.
- Provider admin-probe pilot evidence now carries explicit acceptance markers for `adminProbeOnly`, default-off posture, rollback availability, no public/member provider runtime blocking, no provider runtime enforcement, unchanged provider order/fallback/cache behavior, sanitized-fields-only output, missing accepted operator evidence, and remaining public-launch **NO-GO** items. The offline provider SLA/licensing evidence checker now rejects admin-probe artifacts that claim public/member runtime blocking, provider runtime enforcement, provider order/fallback/cache changes, accepted operator evidence, or public readiness, and it redacts synthetic provider URLs, request ids, tokens, raw payloads, trace markers, credentials, and cache keys. The operator evidence template/bundle workflow now includes `provider_sla_licensing_evidence.json` through the same checker. This is evidence validation only and does not change the admin probe pilot runtime path, public/member provider behavior, provider order/fallback/cache semantics, MarketCache behavior, quota enforcement, auth/RBAC/session behavior, DB schema, frontend source, notifications, broker/order/trade paths, or public-launch **NO-GO** posture.
- Private-beta release/UAT evidence now has a sanitized operator template and offline checker for candidate commit, clean tree, runtime PID/cwd/port owner, guest/auth/admin route boundaries, raw/advice/order leakage, console/network and overflow gates, branch-aware release secret scan, rollback target, and explicit public-launch **NO-GO** fields. This is review plumbing only: it does not run UAT, approve public launch, enable quota/provider enforcement, change auth/RBAC/session behavior, execute DB operations, expose broker/order/trade paths, or send external notifications.
- Portfolio import parse/commit endpoints now return stable sanitized error envelopes for validation, conflict, and internal failures, preserving HTTP status codes and error codes while avoiding raw exception text, stack traces, provider URLs, request ids, account labels, broker refs, import fingerprints, and provider payload markers in member-visible responses. This changes only scoped API error display text and does not alter broker import parsing/commit semantics, dedup, broker-connection linking, portfolio accounting, auth/RBAC, DB schema, frontend source, broker/order/trade behavior, or public-launch **NO-GO** posture.
- Broker import preview/commit API responses now redact broker account refs, order/request/execution IDs, trade UIDs, dedup hashes, import fingerprints, broker URLs, tokens, raw payload markers, and account labels from response artifacts for IBKR XML and legacy broker CSV imports. The redaction is response-boundary evidence only: it does not change broker import execution, dedup, broker-connection linking, portfolio accounting, order/trade execution semantics, auth/RBAC, DB schema, frontend source, notification sends, or public-launch **NO-GO** posture.
...
- Provider circuit low-risk enforcement pilot is now available only for the explicit admin built-in provider validation probe (`/config/data-source/test-builtin`) behind `WOLFYSTOCK_PROVIDER_CIRCUIT_ADMIN_PROBE_PILOT_ENABLED=true`, with `WOLFYSTOCK_PROVIDER_CIRCUIT_ADMIN_PROBE_PILOT_ROLLBACK_ENABLED=true` as the rollback switch. The pilot can block only that admin probe before outbound validation when stored circuit state matches `data_source_validation/admin_provider_probe`; default behavior remains off. This does not change global provider order/fallback/cache behavior, MarketCache TTL/SWR/cold-start behavior, quota enforcement, auth/RBAC/session behavior, DB schema, frontend behavior, notifications, broker/order/trade paths, or public-launch **NO-GO** posture.
...
## 2026-06-11
- Admin/Ops Launch Cockpit now exposes a capability-gated private-beta readiness cockpit at `/admin/launch-cockpit`, backed by an additive `launchCockpit` projection on `GET /api/v1/admin/ops/status`. The projection is admin-only, read-only, advisory-only, sanitized, and keeps public launch **NO-GO**; it does not execute validators, call providers, create quota reservations, change auth/RBAC/session behavior, mutate DB/storage, send notifications, or change provider/runtime/broker behavior.
...
- Options Lab now adds two bounded consumer-safe research visuals in the existing main work region: a read-only expiry payoff sketch for the current observation candidate and a single-expiration IV strike snapshot built only from existing frontend strategy/chain fields. Both visuals fail closed to compact missing-data states, repeat no-advice/no-order/no-broker/no-portfolio-mutation boundaries, and do not change backend contracts, option ranking/gates/payoff math/Greeks/IV calculations, provider/runtime/cache behavior, or existing chain/scenario numeric outputs.
...
- Official macro cache prewarm now reports a sanitized dry-run/write summary
  ...
  to prewarm when activation cache readiness is blocked, so operators see
  missing WALCL/Fed-liquidity coverage before any cache rows are written,
  without exposing provider payloads or changing provider routing, MarketCache
  TTL/SWR, scoring, API, or frontend behavior.
...
- **Docs domain consolidation** - Added current domain entry lanes under
  ...
  to `docs/archive/frontend/`. Updated docs indexes and live references. This
...
- 🧭 **Docs governance archive pruning** - Slimmed active docs navigation by
  ...
  `docs/archive/audits/frontend/`, moving point-in-time QA reports into
  `docs/archive/qa/`, archiving the transitional UI replacement map now under
  `docs/archive/frontend/`, and deleting the generated design scan
  artifact. Added
...
- 🧹 **Phase F archive docs consolidation** — Consolidated the stale
  `docs/archive/architecture/phase-f/` review/index/handoff cluster by
...
- 🧭 **Rotation evidence snapshot source-confidence metadata** — Market rotation radar `rotationStateEvidence` packets now include an additive `evidenceSnapshot` block built from the landed source-confidence contract plus existing theme/breadth/volume/persistence/synchronization fields. The snapshot surfaces capped per-theme and per-signal evidence metadata for relative strength, breadth, volume, persistence, and VWAP participation, including coverage, degraded-signal counts, and explicit `fallback` / `stale` / `partial` / `unavailable` posture without changing existing `rotationScore`, `confidence`, `stage`, theme sorting, provider runtime order/fallback behavior, MarketCache behavior, API compatibility for existing fields, scanner behavior, AI routing/weighting, or any Market Overview / Liquidity Monitor / Portfolio / Backtest / Options / Home semantics.
...
- 🌌 **WolfyStock 视觉宪法第二、三阶段落地到核心工作区页面** — `apps/dsa-web` 继续按前端视觉宪法审计推进页面级收敛；该历史审计现归档于 `docs/archive/frontend/frontend-visual-constitution-audit-2026-04-27.md`。`/chat` 去掉了消息流与底部控制台的居中 `max-w` 囚笼，改成真正跟随 `px-6 md:px-8 xl:px-12` 安全边距展开的三栏研究台；`/portfolio` 移除了 `max-w-[1920px]` 外层容器，并把交易/资金/公司行为、建账、IBKR 只读同步等主 CTA 统一切到高对比白底按钮；`/scanner` 则移除了 `mx-auto + max-w-[1920px]` 壳层，把运行按钮和命中徽标从装饰性绿色收回到白/indigo 体系，同时保留盈亏语义才继续使用绿色。`/settings/system`、`/admin/logs`、个人设置及共享 `Drawer / WorkspacePageHeader` 也同步切到更克制的深空玻璃材质、24px 级圆角、紧凑标题与非绿色 toggle/segment 语义。此次改动不改后端接口、路由权限和业务流程，只继续修正核心工作区的宽屏利用率、材质统一性与语义颜色纪律。
- 🧭 **WolfyStock 视觉宪法第一阶段落地到共享壳层** — `apps/dsa-web` 对共享 `Layout/Nav` 做了一次严格限域收敛：`Shell`、`PreviewShell`、`SidebarNav` 与 `index.css` 现在统一改成真正的 edge-to-edge 外层壳层，去掉共享 masthead / route frame 的居中限宽，统一只保留安全边距；顶部导航、移动抽屉、预览壳层与确认对话框同步切到更克制的深色玻璃材质、边框提亮式 hover/active 反馈和中性 CTA 色，移除旧的 SpaceX 式霓虹渐变品牌强调。配套新增的前端视觉宪法审计现归档于 `docs/archive/frontend/frontend-visual-constitution-audit-2026-04-27.md`，保留待处理的页面级 `mx-auto / max-w-* / 绿色误用 / 过大字号` 违宪点阶段记录。
...
- 🧹 **后端大审计仓库收敛（文档与审计产物治理）** — 将根目录一次性审计报告 `backend-final-audit-report.*` 与 `backend-frontend-global-audit-report.*` 归档到 `docs/archive/architecture/audits/`，删除本地切片交接产物 `slice_report_*.json`，并在 `README.md` 中补充当前项目文档真源入口，减少仓库根目录噪音，降低后续维护时的检索成本。
