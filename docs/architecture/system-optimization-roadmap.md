...
| P3 | Root analysis wrapper cleanup | `SKILL.md` | Cleanup completed. Direct examples now use `src.core.pipeline.StockAnalysisPipeline` and `src.core.market_review.run_market_review`. | Delivered maintainability gain | Low | Small | No | Completed | No |
...
| WS7: Cleanup and deprecation | Remove proven-dead aliases/wrappers and archive non-runtime assets | legacy WebUI startup wrappers, strategy wrappers, smoke wrapper cleanup, `sources/` archive, root analysis wrapper cleanup | no coexistence teardown | After parity/observation window |
