...
class ExecutionLogServiceTestCase(unittest.TestCase):
    ...
    def test_record_market_overview_fetch_persists_panel_audit_fields(self) -> None:
        ...
        self.assertRegex(str(business["traceId"]), r"^trace:sha256:[0-9a-f]{16}$")
        self.assertRegex(str(business["requestId"]), r"^request:sha256:[0-9a-f]{16}$")
        self.assertNotIn("trace-volatility", str(business))
        self.assertNotIn("req-volatility", str(business))
