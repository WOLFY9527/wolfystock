...
LEGACY_SERVE_FLAGS = tuple(f"--{name}" for name in ("webui", "webui-only"))
LEGACY_WEBUI_WRAPPER = "webui" + ".py"
...
class WS7CleanupTestCase(unittest.TestCase):
    ...
    def test_main_help_exposes_only_canonical_serve_flags(self) -> None:
    ...
    def test_webui_wrapper_file_is_removed(self) -> None:
        self.assertFalse((REPO_ROOT / LEGACY_WEBUI_WRAPPER).exists())
