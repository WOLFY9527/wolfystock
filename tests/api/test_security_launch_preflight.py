...
class SecurityLaunchPreflightTestCase(unittest.TestCase):
    ...
    def test_coarse_fallback_disable_staging_rehearsal_evidence_is_complete(self) -> None:
        ...
        self.assertTrue(evidence["backend_admin_routes_explicit_capability_classified"])
        self.assertTrue(evidence["frontend_admin_gates_capability_based"])
        self.assertTrue(evidence["frontend_admin_missing_capabilities_fail_closed"])
        self.assertEqual(evidence["operator_pilot_evidence_path"], "security_operator_acceptance")
