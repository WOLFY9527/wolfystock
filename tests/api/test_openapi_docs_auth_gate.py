...
def test_root_docs_share_production_ingress_fail_closed_invariant(
    tmp_path: Path,
    ...
    with pytest.raises(RuntimeError, match="Production requires ADMIN_AUTH_ENABLED=true"):
        create_app(static_dir=tmp_path / "missing-static")
...
def test_existing_api_v1_openapi_surface_stays_unsupported_when_auth_disabled(
    ...
    response = docs_auth_disabled_client.get("/api/v1/openapi.json", follow_redirects=False)
