...
SECTIONS = [
    ...
    ManualSection(
        ...
        body=(
            ...
            "projection. Release containers map BuildKit `amd64` and `arm64` to the reviewed CPython 3.11 Linux x86_64 "
            "and aarch64 runtime projections, respectively. Their dependency builder reuses the same locked installer "
            "with `--no-deps --require-hashes --no-build-isolation`; requirements intent, development locks, uv, missing "
            "architectures, and unsupported architectures cannot enter that install path. Unsupported target/profile "
            "combinations fail before installation. Online and offline bootstrap "
