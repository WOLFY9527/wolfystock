...
def _finding(field: str, reason_code: str) -> Finding:
...
def _load_json_file(path: Path) -> tuple[Any | None, list[dict[str, str]]]:
    try:
        ...
        return None, [_finding("$", "artifact_read_failed")]
...
def _missing_fields(category: dict[str, Any], required_fields: tuple[str, ...]) -> list[str]:
    return [field for field in required_fields if field not in category]
