...
ROUTE_GROUP_SPECS = [
    ...
    ("scanner.member_surface", "scanner", "authenticated_user", None, "^/api/v1/scanner(?:/(?:readiness|run|runs|themes|strategy-simulation|runs/\\{run_id\\}(?:/research-overlay)?))?$", ["GET", "POST"], None),
    ...
    ("stock.research_surface", "stock", "authenticated_user", None, "^/api/v1/stocks/(?:extract-from-image|parse-import|\\{stock_code\\}/(?:validate|research-packet|evidence|intraday|history|technical-indicators))$", ["GET", "POST"], "Stock research, import, history, and technical-indicator routes are authenticated-member surfaces; only the quote route is explicitly public."),
