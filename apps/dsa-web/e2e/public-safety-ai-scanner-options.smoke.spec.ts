...
appTest.describe('AI and scanner public safety surfaces', () => {
  appTest('AI decision desk shows degraded provider/evidence states without advice or raw internals', async ({ page }) => {
    for (const viewport of viewports) {
      await page.setViewportSize(viewport);
      await signIn(page, '/chat');

      await expect(page.getByRole('heading', { name: 'WOLFY AI 决策台' })).toBeVisible({ timeout: 15_000 });
      ...
      await page.getByPlaceholder('例如：分析 600519 / 茅台现在适合买入吗？（回车发送，Shift+回车换行）').fill('ORCL 走势怎样？');
      ...
      await expect(page.getByText('AI 洞察仅供参考，不构成实质性投资建议。执行交易前请确认风险承受能力。')).toBeVisible();
