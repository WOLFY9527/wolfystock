...
async function installProductRoute(page: Page, currentUser: CurrentUser | null): Promise<ProductAuthHarness> {
  const calls: string[] = [];
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (method === 'GET' && path === '/api/v1/auth/status') {
      return fulfillJson(route, createMockProductAuthStatus(currentUser));
    }
    ...
    return fulfillJson(route, { error: `Unhandled product auth harness route: ${method} ${path}` }, 500);
  });

  return { currentUser, requests };
}

export async function installProductAuthHarness(
  page: Page,
  options: MockProductUserOptions = {},
): Promise<ProductAuthHarness> {
  return installProductRoute(page, createMockProductUser(options));
}

export async function installGuestProductHarness(page: Page): Promise<ProductAuthHarness> {
  return installProductRoute(page, null);
}

export async function openProductRouteWithHarness(
  page: Page,
  path: string,
  options: MockProductUserOptions = {},
): Promise<ProductAuthHarness> {
  const harness = await installProductAuthHarness(page, options);
...
export async function expectRootNonEmpty(page: Page) {
