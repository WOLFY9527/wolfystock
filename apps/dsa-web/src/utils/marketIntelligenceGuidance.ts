...
const ZH_REASON_LABELS: Record<string, string> = {
  ...
  fallback_or_proxy_evidence: '部分可用',
  fallback_proxy_or_observation_only_evidence_present: '当前仅作背景观察',
  ...
  observation_only: '当前仅作背景观察',
  observation_only_discount: '当前仅作背景观察',
  ...
  provider_observation_only: '当前仅作背景观察',
  proxy_context_only: '当前仅作背景观察',
  proxy_only_missing_real_source: '需要更多市场数据',
  proxy_or_observation_only_evidence: '当前仅作背景观察',
  rotation_non_scoring_or_taxonomy_only: '轮动证据仅作分类参考',
  score_contribution_not_allowed: '暂不纳入评分',
...
const EN_REASON_LABELS: Record<string, string> = {
  ...
  fallback_or_proxy_evidence: 'Partly available',
  fallback_proxy_or_observation_only_evidence_present: 'Background only',
  ...
  observation_only: 'Background only',
  observation_only_discount: 'Background only',
  ...
  provider_forbidden_for_use_case: 'Background only',
  provider_observation_only: 'Background only',
  proxy_context_only: 'Background only',
  proxy_only_missing_real_source: 'More market data needed',
  proxy_or_observation_only_evidence: 'Background only',
  rotation_non_scoring_or_taxonomy_only: 'Rotation evidence is taxonomy-only',
  score_contribution_not_allowed: 'Excluded from scoring',
...
export function sanitizeMarketGuidanceCopy(value?: string | null, fallback = '仅供研究观察。'): string {
  ...
  return text
    ...
    .replace(/\bbuy\b/gi, 'positive signal')
    .replace(/\bsell\b/gi, 'reverse signal')
    .replace(/\bstop[-\s]?loss\b/gi, 'fixed-exit reference')
    .replace(/\btake[-\s]?profit\b/gi, 'target-exit reference')
    .replace(/\btarget price\b/gi, 'assumed price')
    .replace(/\badd\b/gi, 'exposure increase')
    .replace(/\breduce\b/gi, 'exposure decrease')
    .replace(/position[-\s]?sizing/gi, 'exposure assumption');
}
...
function localizedEvidenceLabel(value: string | null | undefined, locale: MarketIntelligenceGuidanceLocale): string {
  ...
  const labels: Record<string, string> = {
    ...
    备用或代理证据偏多: 'Partial market data coverage',
...
export function buildMarketDirectionalSummary({
  ...
  const blockingDrivers = uniqueLimited([
    ...
    fallbackPressure > 0 ? (locale === 'en' ? 'partial market data coverage' : '部分市场数据暂不可用') : '',
