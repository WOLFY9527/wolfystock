  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-2">
        <Clock className="size-3.5 text-[color:var(--wolfy-text-muted)]" aria-hidden="true" />
      ...
      <div className="mt-2 grid gap-3" data-testid="scanner-strategy-simulation-body">
        <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
          <div className="min-w-0">
            <span className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-[color:var(--wolfy-text-muted)]">{language === 'en' ? 'Lookback' : '回看'}</span>
            <div className="ui-scroll-x-quiet flex gap-1 rounded-lg border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)] p-0.5">
              {[30, 90, 180].map((value) => (
                <button
                  key={value}
                  type="button"
                  className={`shrink-0 rounded-md px-2.5 py-1 font-mono text-[11px] ${lookbackDays === value ? 'bg-[color:color-mix(in_srgb,var(--sage)_14%,var(--wolfy-surface-input))] font-bold text-[color:var(--wolfy-text-primary)]' : 'text-[color:var(--wolfy-text-muted)] hover:text-[color:var(--wolfy-text-primary)]'}`}
          ...
          <div className="min-w-0">
            <span className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-[color:var(--wolfy-text-muted)]">{language === 'en' ? 'Forward' : '持有'}</span>
            <div className="ui-scroll-x-quiet flex gap-1 rounded-lg border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)] p-0.5">
              {[1, 5, 10, 20].map((value) => (
                <button
                  key={value}
                  type="button"
                  className={`shrink-0 rounded-md px-2.5 py-1 font-mono text-[11px] ${forwardDays === value ? 'bg-[color:color-mix(in_srgb,var(--sage)_14%,var(--wolfy-surface-input))] font-bold text-[color:var(--wolfy-text-primary)]' : 'text-[color:var(--wolfy-text-muted)] hover:text-[color:var(--wolfy-text-primary)]'}`}
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2 rounded-lg border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)] px-3 py-2">
        ...
        {summary && result?.status !== 'insufficient_history' ? (
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6" data-testid="scanner-strategy-simulation-summary">
            {[
              ...
              <div key={label} className="min-w-0 rounded-xl border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)] px-3 py-2">
                <span className="block truncate text-[10px] font-bold uppercase tracking-widest text-[color:var(--wolfy-text-muted)]">{label}</span>
                <span className={`${String(label).includes('Avg') || String(label).includes('Excess') || String(label).includes('平均') || String(label).includes('超额') ? simulationToneClass(parseFirstNumericValue(value)) : 'font-mono text-[color:var(--wolfy-text-primary)]'} block truncate text-sm`}>{value}</span>
              </div>
            ))}
          </div>
        ) : null}
        ...
        {result?.runs.length ? (
          <div className="overflow-x-auto no-scrollbar rounded-xl border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)]" data-testid="scanner-strategy-simulation-runs">
            <table className="min-w-[720px] w-full text-left text-[11px]">
              ...
              <tbody>
                {result.runs.map((item) => (
                  <tr key={item.runId} className="border-b border-[color:var(--wolfy-border-subtle)] text-[color:var(--wolfy-text-secondary)]">
                    <td className="p-2 font-mono text-[color:var(--wolfy-text-primary)]">#{item.runId}</td>
                    <td className="p-2 font-mono">{item.selectedCount}</td>
                    <td className="p-2 font-mono">{item.rejectedCount}</td>
                    <td className="max-w-[180px] truncate p-2 font-mono text-[color:var(--wolfy-text-primary)]" title={item.selectedSymbols.join(', ')}>{item.selectedSymbols.join(', ') || '--'}</td>
        ...
        {result?.symbols.length ? (
          <div className="overflow-x-auto no-scrollbar rounded-xl border border-[color:var(--wolfy-border-subtle)] bg-[var(--wolfy-surface-input)]" data-testid="scanner-strategy-simulation-symbols">
