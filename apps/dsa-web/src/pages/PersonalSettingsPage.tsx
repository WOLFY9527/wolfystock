...
type PersonalSettingsCopy = {
  consoleEyebrow: string;
  pageTitle: string;
  heroSummarySignedIn: string;
  heroSummaryGuest: string;
  guestTitle: string;
  ...
  guestStatus: string;
  authReadyState: string;
  authLimitedState: string;
  signInRequired: string;
  localOnlyState: string;
  loadingTargets: string;
  targetsReady: string;
  notificationsPaused: string;
  accountCardTitle: string;
  accountCardBody: string;
  guestCardTitle: string;
  guestCardBody: string;
  securityCardTitle: string;
  securityCardBodyReady: string;
  securityCardBodyLocked: string;
  securityGuestBody: string;
  sessionCardTitle: string;
  sessionCardBody: string;
  boundaryDisclosureTitle: string;
  boundaryDisclosureSummary: string;
  boundaryDisclosureBody: string;
  preferenceDisclosureTitle: string;
  preferenceDisclosureSummary: string;
  preferenceDisclosureBody: string;
  guestNotificationHint: string;
  notificationHelper: string;
  signedInHint: string;
  railEyebrow: string;
  railTitle: string;
  railBody: string;
  railLinkPrimary: string;
  railLinkSecondary: string;
};

const PERSONAL_SETTINGS_COPY: Record<'en' | 'zh', PersonalSettingsCopy> = {
  en: {
...
function SettingsChoiceRow<T extends string>({
  ...
  return (
    <div className={SETTINGS_ROW_CLASS}>
      ...
      <div className="min-w-0">
        <fieldset className="m-0 flex flex-wrap gap-2 border-0 p-0" aria-label={title}>
          {options.map((option) => {
...
function PersonalSettingsProfileHeader({
  copy,
  loggedIn,
  signedInName,
  statusItems,
}: {
  copy: PersonalSettingsCopy;
  loggedIn: boolean;
  signedInName: string;
...
function SignedInNotificationSettingsPanel({
  copy,
  t,
}: {
  copy: PersonalSettingsCopy;
  ...
  const [notificationState, dispatchNotificationAction] = useReducer(
    notificationPreferencesReducer,
    undefined,
    () => createInitialNotificationPreferencesState(true),
  );
  const {
    email,
    emailEnabled,
  ...
  const handleSaveNotificationPreferences = async () => {
    dispatchNotificationAction({ type: 'save-started' });
    try {
      const prefs = await authApi.updateNotificationPreferences(
        toNotificationPreferenceUpdatePayload(notificationState),
      );
      dispatchNotificationAction({
        type: 'save-succeeded',
        prefs,
        notice: t('settings.personalNotificationTargetsSaved'),
      });
    } catch (requestError) {
      dispatchNotificationAction({
        type: 'save-failed',
  ...
  return (
    <div className={cn(SETTINGS_PANEL_CLASS, 'min-w-0 space-y-3')}>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-3 text-xs text-[color:var(--wolfy-text-secondary)]">
            <input
              ...
              onChange={(event) => dispatchNotificationAction({ type: 'email-enabled-changed', value: event.target.checked })}
          ...
          <label className="block">
            ...
            <input
              ...
              value={email}
              onChange={(event) => dispatchNotificationAction({ type: 'email-changed', value: event.target.value })}
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-3 text-xs text-[color:var(--wolfy-text-secondary)]">
            <input
              ...
              onChange={(event) => dispatchNotificationAction({ type: 'discord-enabled-changed', value: event.target.checked })}
              disabled={loading || saving}
      ...
      {notice ? (
        <p className="text-xs leading-5 text-[hsl(var(--accent-positive-hsl))]">{notice}</p>
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <TerminalButton
          ...
          disabled={loading || saving}
        >
          {saving ? t('settings.personalNotificationSaving') : t('settings.personalNotificationSaveAction')}
        </TerminalButton>
      </div>
    </div>
  );
...
function PersonalNotificationSettingsSection({
  copy,
  loggedIn,
  t,
}: {
  ...
  return (
    ...
    >
      <div data-testid="personal-settings-notification-row" className={SETTINGS_ROW_CLASS}>
        ...
        {loggedIn ? (
          <SignedInNotificationSettingsPanel copy={copy} t={t} />
        ) : (
...
function LocalPreferencesSection({
  ...
  numberFormat,
  onDataDensityChange,
  onMarketColorConventionChange,
  onNumberFormatChange,
  portfolioDisplayCurrency,
  portfolioDisplayCurrencySaved,
  onPortfolioDisplayCurrencyChange,
}: {
  copy: PersonalSettingsCopy;
  t: (key: string, vars?: Record<string, string | number | undefined>) => string;
  dataDensity: (typeof DATA_DENSITY_OPTIONS)[number]['value'];
  marketColorConvention: MarketColorConvention;
  numberFormat: (typeof NUMBER_FORMAT_OPTIONS)[number]['value'];
  onDataDensityChange: (nextValue: (typeof DATA_DENSITY_OPTIONS)[number]['value']) => void;
  onMarketColorConventionChange: (nextValue: MarketColorConvention) => void;
  onNumberFormatChange: (nextValue: (typeof NUMBER_FORMAT_OPTIONS)[number]['value']) => void;
  portfolioDisplayCurrency: PortfolioDisplayCurrency;
  portfolioDisplayCurrencySaved: boolean;
  onPortfolioDisplayCurrencyChange: (currency: PortfolioDisplayCurrency) => void;
}) {
  return (
    <SettingsConsoleSection
      id="preferences"
            ...
            {PORTFOLIO_DISPLAY_CURRENCY_OPTIONS.map((currency) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onPortfolioDisplayCurrencyChange(currency)}
                  aria-pressed={active}
                >
                  {currency}
                </button>
...
const PersonalSettingsPage: React.FC = () => {
  ...
  const { loggedIn, currentUser } = useProductSurface();
  ...
  const copy = PERSONAL_SETTINGS_COPY[language];
  ...
  const statusItems = [
    ...
    },
    {
      ...
      value: loggedIn ? copy.targetsReady : copy.notificationsPaused,
    },
    {
        ...
        <section className="flex min-h-0 min-w-0 flex-col gap-4">
          ...
          >
            <ConsoleBoard data-testid="personal-settings-primary-board">
              ...
              <PersonalNotificationSettingsSection copy={copy} loggedIn={loggedIn} t={t} />
              ...
              <LocalPreferencesSection
                ...
                onMarketColorConventionChange={setMarketColorConvention}
                onNumberFormatChange={setNumberFormat}
                portfolioDisplayCurrency={portfolioDisplayCurrency}
                portfolioDisplayCurrencySaved={portfolioDisplayCurrencySaved}
                onPortfolioDisplayCurrencyChange={handlePortfolioDisplayCurrencyChange}
              />
            </ConsoleBoard>
