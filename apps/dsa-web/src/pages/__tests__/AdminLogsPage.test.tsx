...
describe('AdminLogsPage', () => {
  ...
  it('defaults to business events and does not show raw step names as the main list', async () => {
    ...
    const purposeLine = screen.getByTestId('admin-logs-purpose-line');
    expect(purposeLine).toHaveTextContent('页面用途');
    expect(purposeLine).toHaveTextContent('定位失败与审计线索');
    expect(purposeLine).not.toHaveTextContent('当前状态');
    expect(purposeLine).not.toHaveTextContent('下一步');
    expect(purposeLine).not.toHaveTextContent('1 个失败 / 6 条记录');
    expect(purposeLine).not.toHaveTextContent('数据源响应超时：请检查数据源配置与上游状态。');
    ...
    const filterBar = screen.getByTestId('admin-logs-filter-bar');
    const mainQueue = screen.getByRole('tabpanel');
    const healthSummary = screen.getByTestId('admin-logs-health-summary');
    const operatorIssues = screen.getByTestId('admin-logs-operator-issue-rollup');
    const dataGaps = screen.getByTestId('admin-logs-data-missing-section');
    const storageAdvisory = screen.getByTestId('admin-logs-storage-disclosure');
    expect(filterBar.compareDocumentPosition(mainQueue) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(mainQueue.compareDocumentPosition(healthSummary) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(healthSummary.compareDocumentPosition(operatorIssues) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(operatorIssues.compareDocumentPosition(dataGaps) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(dataGaps.compareDocumentPosition(storageAdvisory) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    await waitFor(() => {
