...
describe('PersonalSettingsPage', () => {
  ...
  it('shows guest-only sign-in guidance without system links', () => {
    ...
    const summaryStrip = screen.getByTestId('personal-settings-summary-strip');
    ...
    expect(summaryStrip).toHaveAttribute('data-linear-primitive', 'status-strip');
    expect(summaryStrip.children).toHaveLength(3);
    expect(screen.getByTestId('personal-settings-profile-header')).toBeInTheDocument();
    expect(screen.getByTestId('personal-settings-account-section')).toBeInTheDocument();
    expect(screen.getByTestId('personal-settings-security-section')).toBeInTheDocument();
    expect(screen.getByTestId('personal-settings-notifications-section')).toBeInTheDocument();
    ...
    expect(screen.getByTestId('personal-settings-account-section')).toHaveTextContent('账户');
    expect(screen.getByTestId('personal-settings-security-section')).toHaveTextContent('安全');
    expect(screen.getByTestId('personal-settings-notifications-section')).toHaveTextContent('通知');
    expect(screen.getByTestId('personal-settings-privacy-section')).toHaveTextContent('隐私与会话');
    expect(screen.getByTestId('personal-settings-preferences-section')).toHaveTextContent('本地偏好');
    ...
    expect(screen.getByRole('link', { name: '去登录' })).toHaveAttribute('href', '/login?redirect=%2Fsettings');
  ...
  it('keeps admin console links out of personal settings content', async () => {
    ...
    expect(screen.queryByTestId('personal-settings-help-rail')).not.toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 1, name: '账户中心' })).toBeInTheDocument();
    expect(screen.getByTestId('personal-settings-summary-strip').children).toHaveLength(3);
