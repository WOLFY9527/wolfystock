...
describe('HomeSurfacePage', () => {
  ...
  it('renders the signed-in bento dashboard for authenticated users', async () => {
    ...
    const linearShell = screen.getByTestId('home-linear-shell');
    ...
    expect(root).toHaveClass('bg-[#080a0d]');
    ...
    expect(omnibarShell).toHaveClass('w-full', 'shrink-0');
    expect(omnibarShell).not.toHaveClass('mb-8', 'max-w-4xl', 'absolute', 'fixed', 'z-10', '-mt-10');
    expect(omnibar).toHaveClass('flex', 'min-h-11', 'w-full', 'min-w-0', 'gap-2');
    ...
    expect(linearShell).toHaveClass('grid', 'min-w-0', 'rounded-[16px]', 'border', 'border-white/[0.055]', 'bg-[#0d0f13]', 'shadow-none');
    ...
    expect(secondaryStack).toHaveClass('relative', 'min-w-0', 'border-t', 'border-white/[0.055]', 'bg-white/[0.012]', 'py-5');
    expect(homeSearch).toHaveAttribute('placeholder', '输入代码唤醒 AI (如 ORCL)...');
    expect(homeSearch).toHaveValue('');
    expect(screen.getByTestId('home-bento-omnibar-input-shell')).toHaveClass('overflow-hidden', 'rounded-lg', 'border', 'border-white/[0.07]', 'bg-[#0d1015]');
    ...
    expect(screen.getByTestId('home-bento-analyze-button')).toHaveClass('rounded-lg', 'bg-white/[0.035]', 'border', 'border-white/[0.08]');
    ...
    expect(strategyCard).not.toHaveClass('rounded-[24px]');
    expect(techCard).not.toHaveClass('rounded-[24px]');
    expect(fundamentalsCard).not.toHaveClass('rounded-[24px]');
    ...
    expect(omnibar.compareDocumentPosition(linearShell) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(omnibar.parentElement).toBe(omnibarShell);
    expect(omnibarShell.parentElement).toBe(grid);
    expect(techCard.compareDocumentPosition(strategyCard) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  ...
  it('keeps the Linear-style Home shell when there is no non-test history', async () => {
    ...
    expect(secondaryStack).toHaveClass('relative', 'min-w-0', 'border-t', 'bg-white/[0.012]', 'py-5');
  ...
  it('keeps analysis loading inside the compact Linear workspace', async () => {
    ...
    expect(screen.getByTestId('home-linear-shell')).toBeInTheDocument();
