...
describe('BacktestPage', () => {
  ...
  it('defaults to the point-and-shoot normal workspace', async () => {
    ...
    expect(researchBoundary).toHaveTextContent('研究边界摘要');
    expect(researchBoundary).toHaveTextContent('受边界约束的模型或规则输出');
    expect(researchBoundary).toHaveTextContent('请独立核验适用性');
    expect(researchBoundary).not.toHaveTextContent('OBSERVATION-ONLY 证据摘要');
    expect(researchBoundary.textContent || '').not.toMatch(/observation-only|OBSERVATION-ONLY|交易建议|买入|卖出|持有/i);
    expect(pageShell.textContent || '').not.toMatch(/\bOBSERVATION-ONLY\b|OBSERVATION-ONLY 证据摘要/i);
  ...
  it('renders exactly one compact semantic backtest page heading without internal terms', async () => {
    ...
    const heading = screen.getByRole('heading', { level: 1, name: '回测实验室' });
  ...
  it('shows point-and-shoot pending readiness feedback and prevents duplicate launches', async () => {
    ...
    const pendingButton = await screen.findByRole('button', { name: /正在检查数据就绪度/ });
    ...
    expect(feedback).toHaveTextContent('正在检查数据就绪度');
  ...
  it('keeps professional pending readiness feedback visible and blocks duplicate submissions', async () => {
    ...
    expect(pendingFeedback[0]).toHaveTextContent('正在检查数据就绪度');

    fireEvent.click(pendingButton);
    expect(runRuleBacktest).toHaveBeenCalledTimes(1);
  });

  it('shows point-and-shoot data-readiness blockers without fake metrics', async () => {
    runRuleBacktest.mockResolvedValueOnce(makeRuleRunResponse({
      status: 'completed',
      completedAt: '2026-04-07T08:02:00Z',
  ...
  it('shows execution readiness before a historical run', async () => {
    renderBacktestRoutes();
